        ...
        <view class="result" v-if="opportunity">
          <view class="result-box" :class="{ 'has-opportunity': opportunity.has_opportunity }">
            <view class="result-header">
              ...
              <text class="result-time">{{ lastUpdated || '现在' }}</text>
            </view>
            ...
            <view class="result-details" v-if="opportunity.has_opportunity">
              <view class="detail-item">
                <text class="detail-label">推荐操作</text>
                <text class="detail-value">{{ opportunity.mode === 'positive' ? '正T (先买后卖)' : '反T (先卖后买)' }}</text>
              </view>
              
              <view class="detail-item">
                <text class="detail-label">建议数量</text>
                <text class="detail-value">{{ opportunity.suggested_quantity }}股</text>
              </view>
              ...
              <view class="detail-item" v-if="opportunity.volatility">
                <text class="detail-label">波动率</text>
              ...
              <view class="ai-decision-panel" v-if="opportunity.evaluation_method === 'ai' && opportunity.has_opportunity">
                <view class="ai-decision-header">
                  ...
                  <text class="ai-decision-title">Agent交易决策</text>
                </view>
                
                <view class="ai-decision-body">
                  <text class="ai-decision-message">
                    AI建议{{ opportunity.mode === 'positive' ? '买入' : '卖出' }} {{ opportunity.suggested_quantity }}股 {{ stockInfo.name }}
                    {{ opportunity.mode === 'positive' ? ',待后续上涨卖出' : ',待后续下跌买回' }}
                  </text>
                  
                  <view class="expected-impact" v-if="opportunity.expected_cost_impact">
                    <text class="impact-title">预期成本降低 {{ opportunity.expected_cost_impact.reduction_percentage.toFixed(2) }}%</text>
                    <text class="impact-detail">
                      {{ opportunity.expected_cost_impact.original_cost.toFixed(2) }} → {{ opportunity.expected_cost_impact.new_cost.toFixed(2) }}
                    </text>
                  </view>
                </view>
  ...
  methods: {
    ...
    executeAIDecision(action) {
      if (!this.opportunity || !this.opportunity.has_opportunity || !this.stockInfo.code) {
        this.$toast.warning('无可用的Agent交易决策');
        return;
      }
      
      // 检查交易账户连接
      if (!this.isConnected) {
        this.$toast.warning('请先连接交易账户');
        return;
      }
      ...
      uni.showModal({
        title: 'Agent交易确认',
        content: `确认${action === 'buy' ? '买入' : '卖出'} ${this.stockInfo.name} ${this.opportunity.suggested_quantity}股 @ ¥${this.stockInfo.current_price.toFixed(2)}?`,
        confirmText: '确认执行',
        confirmColor: '#007aff',
        cancelText: '取消',
        success: async (res) => {
