/**
 * Cloudflare Worker - aigupiao.me 路由系统
 * 复制此代码到 Cloudflare Worker 编辑器
 */
...
async function handleAppDomain(pathname, request, corsHeaders) {
  if (pathname === '/test') {
    return createResponse('✅ App域名测试成功 - Worker正常工作', 200, corsHeaders)
  }
...
async function handleAPIDomain(pathname, request, corsHeaders) {
  if (pathname === '/health') {
    return createResponse(JSON.stringify({
      ...
      version: '1.0.0',
      message: '✅ Worker健康检查成功'
    }), 200, corsHeaders, 'application/json')
  }
  
  if (pathname === '/test') {
    return createResponse('✅ API域名测试成功 - Worker正常工作', 200, corsHeaders)
  }
...
async function handleMobileDomain(pathname, request, corsHeaders) {
  if (pathname === '/test') {
    return createResponse('✅ Mobile域名测试成功 - Worker正常工作', 200, corsHeaders)
  }
...
async function handleAdminDomain(pathname, request, corsHeaders) {
  if (pathname === '/test') {
    return createResponse('✅ Admin域名测试成功 - Worker正常工作', 200, corsHeaders)
  }
...
async function handleMainDomain(pathname, request, corsHeaders) {
  if (pathname === '/test') {
    return createResponse('✅ 主站域名测试成功 - Worker正常工作', 200, corsHeaders)
  }
