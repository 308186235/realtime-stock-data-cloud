#!/bin/bash
# 阿里云ECS OneDrive自动化部署脚本
# 账号：308186235@qq.com

set -e

echo "🚀 阿里云ECS OneDrive自动化部署开始..."
...
# 安装基础依赖
echo "🔧 安装基础依赖..."
sudo apt install -y curl wget unzip python3 python3-pip fuse3 ca-certificates
...
def get_positions():
    ...
    try:
        ...
        data['_metadata'] = {
            ...
            "source": "aliyun_ecs_onedrive_mount",
            "server": "阿里云ECS",
            "network": "中国大陆高速网络"
        }
...
if __name__ == '__main__':
    logging.info("🚀 启动阿里云ECS OneDrive API服务...")
...
[Unit]
Description=OneDrive Mount Service for Aliyun ECS
...
[Unit]
Description=OneDrive API Service for Aliyun ECS
...
# 获取服务器信息
SERVER_IP=$(curl -s ifconfig.me || curl -s ipinfo.io/ip || echo "获取IP失败")

echo "✅ 阿里云ECS OneDrive部署完成！"
echo ""
echo "📋 下一步操作："
echo "1. 配置OneDrive授权:"
echo "   rclone config"
