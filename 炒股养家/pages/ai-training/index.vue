...
try {
    ...
    if (typeof window !== 'undefined' && window.echarts) {
        ...
        console.warn('echarts库未加载，图表功能将不可用');
    }
} catch (error) {
...
export default {
    ...
    onLoad(options) {
        // 检查是否从分析页面跳转过来
        let progress, samples, iterations, accuracy, returns, winRate, drawdown;

        if (options && options.from === 'analysis') {
            // 从全局状态读取数据
            const app = getApp();
            const learningData = app.globalData.learningData || {};

            progress = learningData.progress;
            samples = learningData.samples;
            iterations = learningData.iterations;
            accuracy = learningData.accuracy;
            returns = learningData.returns;
            winRate = learningData.winRate;
            drawdown = learningData.drawdown;
        } else if (options) {
            // 兼容旧的URL参数方式
            progress = parseFloat(options.progress);
            samples = parseInt(options.samples);
            iterations = parseInt(options.iterations);
            accuracy = parseFloat(options.accuracy);
            returns = parseFloat(options.returns);
            winRate = parseFloat(options.winRate);
            drawdown = parseFloat(options.drawdown);
        }
    ...
    methods: {
        ...
        // 开始模拟训练进度更新
        startProgressSimulation(model) {
            if (model.progressInterval) {
                clearInterval(model.progressInterval);
            }

            model.progressInterval = setInterval(() => {
                if (model.status === 'training' && model.progress < 100) {
                    // 模拟进度增长
                    model.progress += Math.random() * 2; // 每次增长0-2%
