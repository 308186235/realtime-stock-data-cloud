...
# 测试函数
def test_double_black_crows_detector():
    """测试双飞乌鸦检测器"""
    import numpy as np
    ...
    # 在高位创建双飞乌鸦形态
    crow_idx = 70
    close[crow_idx-1] = close[crow_idx-2] * 1.01  # 第一根小阴线或阳线
    close[crow_idx] = close[crow_idx-1] * 0.98    # 第二根阴线
    
    # 后续确认
    close[crow_idx+1] = close[crow_idx] * 0.97    # 第三根确认阴线
    
    # 生成OHLCV数据
    high = close * (1 + np.random.uniform(0, 0.015, size=len(close)))
    low = close * (1 - np.random.uniform(0, 0.015, size=len(close)))
    open_prices = close * (1 + np.random.uniform(-0.01, 0.01, size=len(close)))
    
    # 调整双飞乌鸦形态的开盘收盘价
    # 第一根K线
    open_prices[crow_idx-1] = close[crow_idx-2]  # 平开
    high[crow_idx-1] = max(open_prices[crow_idx-1], close[crow_idx-1]) * 1.02  # 长上影线
    
    # 第二根K线 - 高开低走
    open_prices[crow_idx] = close[crow_idx-1] * 1.01  # 高开
    close[crow_idx] = open_prices[crow_idx] * 0.96    # 低走
    
    # 成交量
    volume = np.random.uniform(1000, 2000, size=len(close))
    volume[crow_idx] = volume[crow_idx-1] * 1.7  # 第二根K线放量
    ...
    # 可视化结果
    detector.visualize_pattern(df, signals, pattern_details, "双飞乌鸦测试数据分析")
    
    return df, signals, pattern_details

if __name__ == "__main__":
    test_double_black_crows_detector() 
