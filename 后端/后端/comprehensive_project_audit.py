    ...
    def _check_api_key_exposure(self):
        ...
        if os.path.exists("api_keys.json"):
            exposed_keys.append({
                ...
                "issue": "API密钥明文存储",
                "severity": "HIGH",
                "description": "生产环境API密钥process.env.STOCK_API_KEY暴露在代码中"
            })
        ...
        for file_path in frontend_files:
            if os.path.exists(file_path):
                try:
                    ...
                    if "process.env.STOCK_API_KEY" in content:
                        exposed_keys.append({
                            "file": file_path,
                            "issue": "前端代码包含API密钥",
                            "severity": "CRITICAL",
        ...
        for file_path in supabase_files:
            if os.path.exists(file_path):
                try:
                    ...
                    # 检查硬编码的数据库URL和密钥
                    if "process.env.SUPABASE_URL" in content:
                        db_issues.append({
                            "file": file_path,
                            "issue": "数据库URL硬编码",
                            "severity": "MEDIUM",
                    ...
                    if "process.env.DATABASE_PASSWORD" in content:
                        db_issues.append({
                            "file": file_path,
                            "issue": "数据库密码明文存储",
                            "severity": "CRITICAL",
        ...
        for config_file in config_files:
            if os.path.exists(config_file):
                try:
                    ...
                    if 'JWT_SECRET_KEY = os.getenv("JWT_SECRET_KEY", "process.env.JWT_SECRET_KEY")' in content:
                        config_issues.append({
                            "file": config_file,
                            "issue": "JWT密钥使用默认值",
                            "severity": "HIGH",
