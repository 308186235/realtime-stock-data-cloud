    ...
    def _fix_database_password_exposure(self):
        ...
        for file_path in files_to_fix:
            if os.path.exists(file_path):
                ...
                # 替换硬编码的配置
                replacements = [
                    ('self.url = "https://process.env.SUPABASE_URL"', 
                     'self.url = os.getenv("SUPABASE_URL")'),
                    ('self.key = "process.env.SUPABASE_ANON_KEY
                     'self.key = os.getenv("SUPABASE_ANON_KEY")'),
                    ('self.service_role_key = "process.env.SUPABASE_ANON_KEY
                     'self.service_role_key = os.getenv("SUPABASE_SERVICE_ROLE_KEY")'),
                    ('self.db_url = "postgresql://postgres:[YOUR_PASSWORD]@db.process.env.SUPABASE_URL:5432/postgres"',
...
"""
    ...
    def _update_config_files(self):
        ...
        if os.path.exists(settings_file):
            ...
            # 替换默认JWT密钥
            updated_content = content.replace(
                'JWT_SECRET_KEY = os.getenv("JWT_SECRET_KEY", "process.env.JWT_SECRET_KEY")',
