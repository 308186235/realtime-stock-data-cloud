...
class ServerConfigWizard:
    ...
    def _collect_server_info(self):
        ...
        # API密钥
        api_key = input("API密钥 [process.env.STOCK_API_KEY]: ").strip()
        if not api_key:
...
def main():
    ...
    if choice == "1":
        ...
        if preset_choice in presets:
            ...
            # 仍需要输入敏感信息
            print("\n🔐 请输入敏感信息:")
            wizard.config_data['api_key'] = input("API密钥 [process.env.STOCK_API_KEY]: ").strip() or "process.env.STOCK_API_KEY"
