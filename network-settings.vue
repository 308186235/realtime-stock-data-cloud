...
export default {
  ...
  data() {
    return {
      ...
      // 预设地址
      presetUrls: [
        { name: '生产环境', url: 'https://aigupiao.me' },
        { name: '本地测试', url: 'http://localhost:8081' },
        { name: '局域网', url: 'http://192.168.1.100:8081' }
      ],
  ...
  methods: {
    ...
    loadSettings() {
      try {
        ...
        if (settings) {
          ...
          this.apiBaseUrl = config.apiBaseUrl || 'https://aigupiao.me';
    ...
    resetSettings() {
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            this.apiBaseUrl = 'https://aigupiao.me';
            this.timeout = 10;
