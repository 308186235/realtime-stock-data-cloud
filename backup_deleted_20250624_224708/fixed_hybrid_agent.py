...
class FixedHybridTradingAgent(AutoCleanupTradingAgent):
    def __init__(self):
        super().__init__()
        self.winapi_data = None
        self.export_data = None
        
    def navigate_to_trading_interface(self):
        """导航到正确的交易界面（F1买入界面）"""
        print(" 导航到交易界面...")
        
        # 按F1进入买入界面（这里可以正确获取持仓信息）
        print("   按F1进入买入界面...")
        win32api.keybd_event(0x70, 0, 0, 0)  # F1按下
        time.sleep(0.05)
        win32api.keybd_event(0x70, 0, win32con.KEYEVENTF_KEYUP, 0)  # F1释放
        time.sleep(2)  # 等待界面切换
        
        print("    已切换到买入界面（正确的持仓数据界面）")
        return True
    ...
    def sync_virtual_from_export(self):
        ...
        # 2. 导航到正确的交易界面
        print("\\n 2. 导航到正确的交易界面...")
        self.navigate_to_trading_interface()
...
# 便捷接口
def create_fixed_hybrid_agent():
    """创建修复版混合数据源Agent"""
    return FixedHybridTradingAgent()

# 测试函数
if __name__ == "__main__":
    print(" 测试修复版混合数据源Agent")
    print("=" * 60)
    
    # 创建修复版Agent
    agent = create_fixed_hybrid_agent()
    print(" 修复版Agent创建成功")
    ...
    if result:
        ...
        if hasattr(agent, 'export_data') and agent.export_data:
            print(" 修复验证: 持仓数据获取成功")
        else:
            print(" 持仓数据获取可能有问题")
    else:
